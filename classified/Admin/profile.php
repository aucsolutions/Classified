<?php
	...
	$adDetail = $data->select ( "Account" , "*" , array ( "AccountID" => $_GET["id"] ) ) ;
	$adDetail = $adDetail[0] ;
	...
	if ( $_POST )
	{
		...
		$data->update ( "Account" , $dataValues , array ( "AccountID" => $_GET["id"] ) ) ;
					...
					<form class="application_form" action="" method="post" enctype="multipart/form-data" onsubmit="return validateForm ( ) ;">
						<table cellpadding="2" cellspacing="1" width="70%">
							...
							<tr>
								...
								<td width="71%" valign="top"><?php echo $adDetail["EmailAddress"] ?></td>
							</tr>
							...
							?>
							<tr>
								<td width="30%" class="form_title"><label for="address_country">Country</label></td>
								<td>
									<select name="Country_Setting_" sch_req="1" id="address_country" sch_msg="<?php echo $lang["lang_header"]["register_form"]["str_your_country"] ?>" >
										...
										<?php
											foreach ( $countries as $country )
												if ( $adDetail["Country"] == $country["CountryName"] )
