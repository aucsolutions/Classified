<?php
	...
	if ( $_POST )
	{
		...
		{
			$admin_detail = $data->select ( "Account" , "*" , array ( "Password" => $postArray["OLD_Password"] , "AccountID" => intval ( $_SESSION["login_account_id"] ) ) ) ;
			if ( ! empty ( $admin_detail ) )
			{
				$postArray["New_Password_1"] = $postArray["New_Password_1"] ;
				$data->update ( "Account" , array ( "Password" => $postArray["New_Password_1"] ) , array ( "Password" => $postArray["OLD_Password"] , "AccountID" => intval ( $_SESSION["login_account_id"] ) ) ) ;
				
				$_SESSION["str_system_message"] = "Your password changed successfully." ;
						...
						<table cellpadding="2" cellspacing="1" width="70%">
							...
							<tr>
								...
								<td width="71%" valign="top"><input type="text" name="OLD_Password" size="50" maxlength="100" class="form_text" value=""  sch_req="1" sch_msg="Admin Email address"  />
									<img src="../Admin/images/icons/help.png" onclick="showHelp(this)" style="cursor:pointer" />
									<div class="div_help">
										Old Password
								</div></td>
							</tr>
