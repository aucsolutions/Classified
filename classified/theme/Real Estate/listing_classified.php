...
<div id="main">
				...
				<tr>
					<td valign="top" width="25%">
							<div class="innerLeftNav">
								<div id="search-nav"> 
									<div class="heading">
										<?php echo $lang["lang_listing"]["str_current_matches"] ?> (<?php echo intval ( $total_ads ) ?>)
									</div>
	...
	<?php 
		...
		if ( intval ( $selected_category_detail["HeadCategoryID"] ) > 0 )
		{
			...
			echo "<li><a href='".base_url."category/".get_sef_url ( $head_category_detail["CategoryID"] , "Category" )."/'>". $head_category_detail["CategoryName"]."</a>($total_records_sub_element)" ;
			...
			if ( ! empty ( $sub_cats ) )
			{
				foreach ( $sub_cats as $cat )
				{
					...
					$data->set_like ( $like_array ) ;
					$total_records_sub_element = $data->count_record ( "Classified" , $array_to_db ) ;
					echo "<li><a href='".base_url."category/".get_sef_url ( $cat["CategoryID"] , "Category" )."/'>". $cat["CategoryName"]."</a>($total_records_sub_element) " ;
					if ( intval ( $cat["CategoryID"] ) == intval ( $selected_category_detail["CategoryID"] ) )
					{
						...
						if ( ! empty ( $sub_cat_selected ) )
						{
							...
							foreach ( $sub_cat_selected as $sub_sub_cat )
							{
								...
								$data->set_like ( $like_array ) ;
								$total_records_sub_element = $data->count_record ( "Classified" , $array_to_db ) ;
								echo "<li><a href='".base_url."category/".get_sef_url ( $sub_sub_cat["CategoryID"] , "Category" )."/'>". $sub_sub_cat["CategoryName"]."</a>($total_records_sub_element) </li>" ;
							}
							echo "</ul>" ;
						}
					}
		...
		else
		{
			...
			if ( intval ( $selected_category_detail["CategoryID"] ) > 0 )
				echo "<li><a href='".base_url."category/".get_sef_url ( $selected_category_detail["CategoryID"] , "Category" ) ."/'>". $selected_category_detail["CategoryName"]."</a>($total_records_sub_element)" ;
			...
			if ( ! empty ( $sub_cats ) )
			{
				...
				foreach ( $sub_cats as $cat )
				{
					...
					$data->set_like ( $like_array ) ;
					$total_records_sub_element = $data->count_record ( "Classified" , $array_to_db ) ;
					echo "<li><a href='".base_url."category/".get_sef_url ( $cat["CategoryID"] , "Category" ) ."/'>". $cat["CategoryName"]."</a>($total_records_sub_element) </li>" ;
	...
	?>
					...
					<td valign="top" width="100%">
										...
										<tr>
											...
											<td align="right" width="50%" nowrap="true">
												<form class="jsonly" name="sort" action="<?php echo base_url ?>search/" method="post">
													...
													<span style="color:#000000;">Sort by&nbsp; </span>
						...
						?>
								...
								<div class="listing_list_view" >
									<table width="100%">
										<tr><td width="18%" align="center"><img src="<?php echo base_url ?>classified_img.php?clsid=<?php echo $classif["AdID"] ?>" alt="No Image"></td>
										
											<td width="82%"><?php echo (intval ( $classif["IsOffer"] ) == 0 ? "<strong>Wanted:</strong> ":"") ?><a class="listing_link" href="<?php echo base_url.get_sef_url ( $classif["AdID"] , "Classified" ) ; ?>/"><strong ><?php echo $classif["AdTitle"] ?></strong></a>
											&nbsp; &nbsp; 
											<span class="listing_price"><?php
												...
												?></span>
											<br>
											<span class="desc"><?php echo substr ( strip_tags($classif["Description"]) , 0 , 300 ); ?>...</span>
											<br>
											<span class="date"><?php echo date( "F j, Y" , strtotime ( $classif["DateAdded"] ) ) ?></span>
			...
			?>
										...
										<div class="adFrameKj">
											<div class="adInnerKj"> <a class="viewAd" href="<?php echo base_url.get_sef_url ( $classif["AdID"] , "Classified" ) ; ?>/" >
												...
												<div>
													...
													<div class="price" style="padding: 15px 15px 0pt;"> 
													<?php
														if ( intval ( $classif["PriceAlternative"] ) > 1 )
															echo $price_alt[intval ( $classif["PriceAlternative"] )] ;
														else
								...
								<?php
										...
										$url_listing = base_url.get_sef_url ( $classif["AdID"] , "Classified" ) ;
								?>
									<div class="listing_list_view" >
									<table width="100%">
										<tr>
											<td width="18%" align="center">
												<a href="<?php echo $url_listing ?>/"><img src="<?php echo base_url ?>classified_img.php?clsid=<?php echo $classif["AdID"] ?>" alt="No Image"></a>
											</td>
										
											<td width="82%"><?php echo (intval ( $classif["IsOffer"] ) == 0 ? "<strong>Wanted:</strong> ":"") ?>
												<a href="<?php echo $url_listing ?>/"><?php echo $classif["AdTitle"] ?></a>
												&nbsp; &nbsp; 
												<span class="listing_price"><?php
													if ( intval ( $classif["PriceAlternative"] ) > 1 )
														echo $price_alt[intval ( $classif["PriceAlternative"] )] ;
			...
			?>
								...
								<div class="pagination_bottom">
									...
									<?php
										...
										{
											echo $lang["lang_listing"]["paging_links"]["str_page"]." : " ;
											$total_pages_calculated = ceil ( $total_ads / $page_size ) ;
											$number_of_pages_to_show = $total_pages_calculated ;
											$start_number_of_pages = 0 ;
											if ( $page_number > 2 )
												echo "<a href='".base_url.$qstring[0].$category_query_string.get_sef_search_listing_url ( $parameters , "p" , "p:".($page_number-1 ) )."/' >".$lang["land_listing"]["paging_links"]["str_previous"]."</a>" ;
											...
											for ( $i = $start_number_of_pages ; $i < $number_of_pages_to_show ; $i++ )
											{
												if ( $page_number == $i )
													...
													echo "<a href='".base_url.$qstring[0].$category_query_string.get_sef_search_listing_url ( $parameters , "p" , "p:$i" )."/' >".($i+1)."</a>" ;
											}
											if ( $total_pages_calculated > ( $page_number + 10 ) )
												echo "<a href='".base_url.$qstring[0].$category_query_string.get_sef_search_listing_url ( $parameters , "p" , "p:".($page_number+10 ) )."/' >".$lang["land_listing"]["paging_links"]["str_next"]."</a>" ;
										}
